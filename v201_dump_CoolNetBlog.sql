...
CREATE TABLE `AdminUser`  (
  ...
  `SmtpPort` int NULL DEFAULT NULL COMMENT '此admin的smtp邮箱服务器端口',
...
CREATE TABLE `Article`  (
  ...
  `CommentType` int NOT NULL DEFAULT 1 COMMENT '评论类型，1直接公开，2经过审核，3不允许评论',
...
CREATE TABLE `ArticleThumbUp`  (
  ...
  `UpTime` datetime NULL DEFAULT NULL COMMENT '点赞时间',
  `Type` int NULL DEFAULT NULL COMMENT '点赞类型：1觉得很赞2有被笑到3不敢苟同',
...
CREATE TABLE `Comment`  (
  `Id` int UNSIGNED NOT NULL AUTO_INCREMENT,
  `SourceId` int UNSIGNED NOT NULL COMMENT '此评论对应的对象表原Id，如：文章是文章表的Id',
  `SourceType` int UNSIGNED NOT NULL COMMENT '此评论对应的对象表类型，1文章，2.。3.。。',
...
CREATE TABLE `CommonThumbUp`  (
  `SourceId` int NOT NULL COMMENT '源Id',
  `SourceType` int NOT NULL COMMENT '源Id所属类型：1\"闲言碎语\"组件表，2.. (文章点赞不使用此表而是专门的文章点赞表ArticleThumbUp)',
...
CREATE TABLE `Gossip`  (
  `Id` int UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '\'闲言碎语\'自增主键',
  `Type` int NOT NULL DEFAULT 1 COMMENT '内容类型。1纯文字2带图片的文字',
  ...
  `StarNumber` int NOT NULL DEFAULT 0 COMMENT '点赞总数',
  PRIMARY KEY (`Id`) USING BTREE
...
CREATE TABLE `Reply`  (
  `Id` int UNSIGNED NOT NULL AUTO_INCREMENT,
  `CommentId` int NOT NULL COMMENT '对应的评论表Id',
...
CREATE TABLE `SiteSetting`  (
  ...
  `TailContent` varchar(500) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '尾部文字内容',
  ...
  `LeaveLimitCount` int NULL DEFAULT 0 COMMENT 'ip一日内允许的留言次数(包括回复、评论)，0为无限制',
